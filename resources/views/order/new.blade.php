...
<div class="content">
    <div class="fade-in">
        <div class="card px-4 py-2">
            ...
            <div class="card-body">
                <div>
                    <form method="POST" action="{{ route('staff.orders.storeCSV') }}" enctype="multipart/form-data" class="prevent-double-click">
                        @csrf

                        <div>
                            <div class="d-flex justify-content-between align-items-center amb-12 border-bottom apb-4">
                                <h4 class="amb-4 mt-0">{{ __('Create Via CSV') }}</h4>
                            </div>
                            ...
                            {{-- <label class="search-label col-form-label">
                                <b>{{ __('Packages Dimensions') }}</b>
                            </label>
                            <div class="row amb-20 amx-n4">
                                <div class="col-12 col-md-5 col-xl-4 apx-4 amb-8">
                                    ...
                                    <input
                                        ...
                                        name="csv_package_length" placeholder="Package Length" step="any" min="0"
                                    />
                                </div>
                                <div class="col-12 col-md-5 col-xl-4 apx-4 amb-8">
                                    <b>{{ __('Width') }}</b>
                                    <input
                                        type="number" class="form-control"
                                        name="csv_package_width" placeholder="Package Width" step="any" min="0"
                                    />
                                </div>
                                <div class="col-12 col-md-5 col-xl-4 apx-4 amb-8">
                                    <b>{{ __('Height') }}</b>
                                    <input
                                        type="number" class="form-control"
                                        name="csv_package_height" placeholder="Package Height" step="any" min="0"
                                    />
                                </div>
                                <div class="col-12 col-md-5 col-xl-4 apx-4 amb-8">
                                    <b>{{ __('Type') }}</b>
                                    <select id="csv_size_type" name="csv_size_type" class="form-control w-75">
                                        @foreach (App\Models\OrderPackage::$sizeName as $value => $name)
                                            <option value="{{ $value }}"
                                                @if (old('csv_size_type') == $value)
                                                    selected="selected"
                                                @endif
                                            >{{ ucfirst($name) }}</option>
                                        @endforeach
                                    </select>
                                    @if ($errors->has('csv_size_type'))
                                        <p class="text-danger mb-0">
                                            {{ $errors->first('csv_size_type') }}
                                        </p>
                                    @endif
                                </div>
                            </div>
                            ...
                            <div class="row amb-20 amx-n4">
                                <div class="col-12 col-md-5 col-xl-4 apx-4 amb-8">
                                    ...
                                    <input
                                        type="number" class="form-control"
                                        name="csv_package_weight" placeholder="Package Weight" step="any" min="0"
                                    />
                                </div>
                                <div class="col-12 col-md-5 col-xl-4 apx-4 amb-8">
                                    <b>{{ __('Type') }}</b>
                                    <select id="csv_weight_type" name="csv_weight_type" class="form-control w-75">
                                        @foreach (App\Models\OrderPackage::$weightName as $value => $name)
                                            <option value="{{ $value }}"
                                                @if (old('csv_weight_type') == $value)
                                                    selected="selected"
                                                @endif
                                            >{{ ucfirst($name) }}</option>
                                        @endforeach
                                    </select>
                                    @if ($errors->has('csv_weight_type'))
                                        <p class="text-danger mb-0">
                                            {{ $errors->first('csv_weight_type') }}
                                        </p>
                                    @endif
                                </div>
                            </div> --}}
                            ...
                            <div class="form-group search-form-group">
                                ...
                                <div class="form-group mb-0">
                                    <button type="submit" class="btn btn-round btn-success w-100">
                                        {{ __('Create Order Via CSV') }}
                                    </button>
                                </div>
                            </div>
            ...
            <div class="card-body">
                <div>
                    <form method="POST" action="{{ route('staff.orders.create') }}" class="form-horizontal prevent-double-click" role="form">
                        @csrf
                        ...
                        <div id="content">
                            <div class="d-flex justify-content-between align-items-center amb-12 border-bottom apb-4">
                                <h4 class="amb-4 mt-0">{{ __('Create Manual - Receiver Information') }}</h4>
                            </div>
                            ...
                            @if (old('product'))
                                @foreach (old('product') as $index => $oldProduct)
                                    <div class="amb-8 apy-8 addition-form mt-3" id="{{ 'product_' . $index }}">
                        ...
                        <div class="search-form-group">
                            ...
                            <div class="search-input text-center text-sm-left">
                                <input class="btn btn-info btn-round" type="submit" value="{{ __('Create Order') }}">
                            </div>
                        </div>
                    </form>
                </div>
...
<script>
    ...
    function addProduct() {
        ...
        $('#content').append(`
            <div class="amb-8 apy-8 addition-form mt-3" id="product_${groupId}">
                <div id="product_${groupId}_form">
                    <div class="row amb-20 amx-n4">
                        <div class="col-12 col-md-5 col-xl-4 apx-4 amb-8">
                            <b>{{ __('Product Type *') }}</b>
                            <select id="product_${groupId}_select" name="product[${groupId}][id]" class="form-control">
                                @foreach ($products as $product)
                                    <option value="{{ $product->id }}"
                                    >{{ $product->name }}</option>
                                @endforeach
