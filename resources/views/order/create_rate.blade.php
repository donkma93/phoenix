                                ...
                                <tbody style="text-align: center">
                                    @foreach ($rates as $index => $rate)
                                        <tr style="text-align: center">
                                            ...
                                            <td> {{ $rate->estimated_days == '1' ? $rate->estimated_days . ' day' : $rate->estimated_days . ' days' }}
                                            </td>
                                            <td>
                                                @if ($rate->attributes)
                                                    @php
                                                        $attributes = is_string($rate->attributes)
                                                            ? json_decode($rate->attributes, true)
                                                            : $rate->attributes;
                                                    @endphp
                                            ...
                                            <td>
                                                ...
                                                <button class="btn btn-info btn-round update-rate""
                                                    data-rate="{{ $rate->id }}">Choose Rate</button>
                                            </td>
                                        </tr>
                                    @endforeach
                                </tbody>
            ...
            $('.update-rate').on('click', function(e) {
                ...
                $.ajax({
                    ...
                    error: function(xhr, status, error) {
                        ...
                        let errorMessage =
                            'Something wrong! Please contact admin for more information!';

                        // Try to get error message from response
                        if (xhr.responseJSON) {
                            // Check for errors array (Laravel validation format)
