...
window.onload = function () {
  var noti = document.getElementById("chat-noti");
    ...
    if (_typeof(data) === "object") {
      if (data.length > 0) {
        ...
        for (var i = 0; i < data.length; i++) {
          var currentUser = data[i].from_user_id == data[i].chat_box.user_id;
          var row = "\n                    <li class=\"mb-2 ".concat(currentUser ? 'd-flex justify-content-end' : '', "\">\n                        <div class=\"").concat(currentUser ? 'bg-primary text-white' : 'bg-light text-dark', " py-1 px-3 rounded-pill d-inline-block\">\n                                ").concat(data[i].message, "\n                        </div>\n                    </li>");
          element.append(row);
        }
      ...
      if (_typeof(data) === "object") {
        if (data.newMessage.length > 0) {
          ...
          for (var i = 0; i < data.newMessage.length; i++) {
            var currentUser = data.newMessage[i].from_user_id == data.newMessage[i].chat_box.user_id;
            var row = "\n                        <li class=\"mb-2 ".concat(currentUser ? 'd-flex justify-content-end' : '', "\">\n                            <div class=\"").concat(currentUser ? 'bg-primary text-white' : 'bg-light text-dark', " py-1 px-3 rounded-pill d-inline-block\">\n                                    ").concat(data.newMessage[i].message, "\n                            </div>\n                        </li>");
            element.append(row);
          }

          document.querySelector("#chat-content").scrollTo(0, document.querySelector("#chat-content").scrollHeight);

          var _noti = document.getElementById("chat-noti");
  ...
  document.getElementById('create_message').addEventListener('keyup', function (e) {
    if (e.key === 'Enter' || e.keyCode === 13) {
      var _message = $('#create_message').val();
